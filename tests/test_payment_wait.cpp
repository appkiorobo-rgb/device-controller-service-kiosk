...
void printPaymentWaitResponse(const PaymentWaitResponse& response) {
    std::cout << std::endl;
    std::cout << "=== Payment Wait Response ===" << std::endl;
    ...
    if (!response.data.empty()) {
        ...
        std::cout << "Data (ASCII): ";
        for (size_t i = 0; i < response.data.size() && i < 64; ++i) {
            char c = static_cast<char>(response.data[i]);
            if (c >= 32 && c < 127) {
                std::cout << c;
            } else {
                std::cout << ".";
...
int main(int argc, char* argv[]) {
    try {
        // 로거 초기화
        logging::Logger::getInstance().initialize("payment_wait_test.log");
        ...
        logging::Logger::getInstance().info("=== Payment Wait Test ===");
        ...
        for (const auto& port : availablePorts) {
            ...
            if (comm.sendPaymentWaitRequest(terminalId, response)) {
                ...
                if (availablePorts.size() > 1) {
                    std::cout << "  Success! Payment wait request completed on " << port << std::endl;
                }
                logging::Logger::getInstance().info("Payment wait request completed on port: " + port);
        ...
        if (success) {
            printPaymentWaitResponse(response);
            std::cout << std::endl << "Payment wait request completed successfully on " << workingPort << "!" << std::endl;
        } else {
            std::cerr << std::endl << "Error: Payment wait request failed on all ports" << std::endl;
            if (!availablePorts.empty()) {
